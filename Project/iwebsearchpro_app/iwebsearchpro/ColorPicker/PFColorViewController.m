...
- (void)viewDidLoad {
    ...
    if (self.colorPicker == nil) {
        ...
        NSLog(@"*** transparent Size : %@", NSStringFromCGRect(transparent.frame));
        ...
        if (IS_IPAD) {
            [colorPicker setFrame:CGRectMake(0, 64, self.view.frame.size.width-300, height)];
        }
        ...
        NSLog(@"*** controlsContainer Size : %@", NSStringFromCGRect(controlsContainer.frame));
        ...
        if (IS_IPAD) {
            [controlsContainer setCenter:CGPointMake(self.view.center.x+150, self.view.frame.size.height-(controlsContainer.frame.size.height/2))];
        } else {
        ...
        NSLog(@"*** controlsContainer Size : %@", NSStringFromCGRect(sliderFrame));
